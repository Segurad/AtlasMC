	...
	public void read(ClientboundLoginSuccess packet, ByteBuf in, ConnectionHandler handler) throws IOException {
		...
		if (propCount > 0) {
			for (int i = 0; i < propCount; i++) {
				String name = StringCodec.readString(in, 64);
				String value = StringCodec.readString(in, 32767);
				String signature = in.readBoolean() ? StringCodec.readString(in, 1024) : null;
