...
public class CorePacketOutDisplayObjective implements PacketCodec<PacketOutDisplayObjective> {

	@Override
	public void deserialize(PacketOutDisplayObjective packet, ByteBuf in, ConnectionHandler handler) throws IOException {
		packet.position = EnumUtil.getByID(DisplaySlot.class, in.readByte());
		packet.objective = StringCodec.readString(in);
	}

	@Override
	public void serialize(PacketOutDisplayObjective packet, ByteBuf out, ConnectionHandler handler) throws IOException {
