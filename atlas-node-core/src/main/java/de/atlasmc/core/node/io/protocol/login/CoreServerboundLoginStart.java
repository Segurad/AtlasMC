...
public class CoreServerboundLoginStart implements PacketCodec<ServerboundLoginStart> {

	@Override
	public void deserialize(ServerboundLoginStart packet, ByteBuf in, ConnectionHandler handler) throws IOException {
		packet.name = StringCodec.readString(in, 16);
		packet.uuid = UUIDCodec.readUUID(in);
	}

	@Override
	public void serialize(ServerboundLoginStart packet, ByteBuf out, ConnectionHandler handler) throws IOException {
