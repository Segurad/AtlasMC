...
public class CorePacketInResourcePack implements PacketCodec<PacketInResourcePack> {
	
	@Override
	public void deserialize(PacketInResourcePack packet, ByteBuf in, ConnectionHandler handler) throws IOException {
		packet.status = EnumUtil.getByID(ResourcePackStatus.class, readVarInt(in));
	}

	@Override
	public void serialize(PacketInResourcePack packet, ByteBuf out, ConnectionHandler handler) throws IOException {
