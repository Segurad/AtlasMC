...
public class CoreMapMeta extends CoreItemMeta implements MapMeta {
	...
	protected static final CharKey
			MAP = CharKey.literal("map"),
			MAP_SCALE_DIRECTION = CharKey.literal("map_scale_direction"),
			MAP_COLOR = CharKey.literal("MapColor"),
			MAP_TRACING_POSITION = CharKey.literal("map_tracking_position"),
			DECORATION = CharKey.literal("Decoration");
	...
	static {
		...
		NBT_FIELDS.getContainer(NBT_DISPLAY).setField(MAP_COLOR, (holder, reader) -> {
			if (holder instanceof MapMeta) {
				((MapMeta) holder).setColor(Color.fromRGB(reader.readIntTag()));
			} else 
				holder.getCustomTagContainer().addCustomTag(reader.readNBT());
		});
