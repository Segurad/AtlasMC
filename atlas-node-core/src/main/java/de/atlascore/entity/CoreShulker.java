...
public class CoreShulker extends CoreMob implements Shulker {
	...
	NBT_ATTACHED_POS_Y = CharKey.literal("APY"),
	NBT_ATTACHED_POS_Z = CharKey.literal("APZ"),
	NBT_ATTACHED_FACE = CharKey.literal("AttachFace"),
	...
	static {
		...
		NBT_FIELDS.setField(NBT_ATTACHED_POS_Y, (holder, reader) -> {
			holder.setAttachedY(reader.readIntTag());
		});
		NBT_FIELDS.setField(NBT_ATTACHED_POS_Z, (holder, reader) -> {
			holder.setAttachedZ(reader.readIntTag());
		});
	...
	public Location getAttachedPosition(Location loc) {
		...
		if (pos == null)
			return null;
		return loc.set(getWorld(), apX, apY, apZ, 0, 0);
	}
