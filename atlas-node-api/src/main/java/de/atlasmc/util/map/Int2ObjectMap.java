 ...
 * @param <V>
 */
public class Int2ObjectMap<V> implements Iterable<V> {
	
	private static final int MIN_SIZE = 32;
	private static final float DEFAULT_REHASH_DOWN = 0.25f;
	private static final float DEFAULT_REHASH_UP = 0.75f;
	...
	public Int2ObjectMap(int mincapacity) {
		this(mincapacity, DEFAULT_REHASH_UP, DEFAULT_REHASH_DOWN);
	}
	
	@SuppressWarnings("unchecked")
	public Int2ObjectMap(int mincapacity, float rehashUp, float rehashDown) {
		if (mincapacity < 0)
	 ...
	 * @return node or null
	 */
	private Node<V> getNode(int hash, int key, boolean create, Node<V> existing) {
	...
	private Node<V> insertAfter(int hash, int key, Node<V> prev, Node<V> existing) {
	...
	private Node<V> insertBefore(int hash, int key, Node<V> next, Node<V> existing) {
		Node<V> node = existing == null ? new Node<>(hash, key) : existing;
		node.next = next;
		if (next.prev != null) {
			next.prev.next = node;
	...
	private class ValueIterator implements Iterator<V> {
		...
		public ValueIterator() {
			this.current = head;
			this.mods = Int2ObjectMap.this.modifikations;
		}
		...
		@Override
		public V next() {
			if (this.mods != Int2ObjectMap.this.modifikations)
		...
		@Override
		public void remove() {
			Int2ObjectMap.this.removeNode(last);
			last = null;
		}
	...
	private class Values extends AbstractCollection<V> {
		...
		@Override
		public void clear() {
			Int2ObjectMap.this.clear();
		}
		
		@Override
		public boolean contains(Object o) {
			return Int2ObjectMap.this.containsValue(o);
		}
