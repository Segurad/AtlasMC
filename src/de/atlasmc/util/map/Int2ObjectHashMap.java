 ...
 * @param <V>
 */
public class Int2ObjectHashMap<V> implements Iterable<V> {
	...
	public Int2ObjectHashMap(int mincapacity) {
		this(mincapacity, DEFAULT_REHASH_UP, DEFAULT_REHASH_DOWN);
	}
	
	@SuppressWarnings("unchecked")
	public Int2ObjectHashMap(int mincapacity, float rehashUp, float rehashDown) {
		if (mincapacity < 0)
	 ...
	 * @return node or null
	 */
	private Node<V> getNode(int hash, int key, V value, boolean create, Node<V> existing) {
	...
	private Node<V> insertAfter(int hash, int key, V value, Node<V> prev, Node<V> existing) {
	...
	private Node<V> insertBefore(int hash, int key, V value, Node<V> next, Node<V> existing) {
		Node<V> node = existing == null ? new Node<>(hash, key, value) : existing;
		node.next = next;
		if (next.prev != null) {
			next.prev.next = node;
	...
	final class Node<E> {
		...
		public Node(int hash, int key, E value) {
			this.hash = hash;
			this.key = key;
			this.value = value;
		}	
	...
	private class ValueIterator implements Iterator<V> {
		...
		public ValueIterator() {
			this.current = head;
			this.mods = Int2ObjectHashMap.this.modifikations;
		}
		...
		@Override
		public V next() {
			if (this.mods != Int2ObjectHashMap.this.modifikations)
		...
		@Override
		public void remove() {
			Int2ObjectHashMap.this.removeNode(last);
			last = null;
		}
