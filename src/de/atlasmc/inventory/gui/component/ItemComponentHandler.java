...
public final class ItemComponentHandler extends AbstractComponentHandler {
	...
	ItemComponentHandler(Component<ItemStack> comp, GUI gui, int slot, int length, int depth, int offsetX, int offsetY, boolean slotwatcher) {
		...
		if (length > comp.getLengthX()) throw new IllegalArgumentException("The display length can not be heigher than the component length!");
		if (depth > comp.getLengthY()) throw new IllegalArgumentException("The display depth can not be heigher than the component depth!");
		this.comp = comp;
		this.inv = null;
		if (slotwatcher) {
