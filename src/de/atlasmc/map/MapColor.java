...
public final class MapColor {
	...
	static {
		...
		for (int i = 0; i < c.length; i++) {
			final int j = i;
			colors[j*4+LEVEL_LOW] = c(c[j][0]*180/255, c[j][1]*180/255, c[j][2]*180/255, LEVEL_LOW, j*4+LEVEL_LOW,j);
			colors[j*4+LEVEL_LOWER] = c(c[j][0]*220/255, c[j][1]*220/255, c[j][2]*220/255, LEVEL_LOWER, j*4+LEVEL_LOWER,j);
			colors[j*4+LEVEL_NORMAL] = c(c[j][0], c[j][1], c[j][2], LEVEL_NORMAL, j*4+LEVEL_NORMAL,j);
			colors[j*4+LEVEL_LOWEST] = c(c[j][0]*135/255, c[j][1]*135/255, c[j][2]*135/255, LEVEL_LOWEST, j*4+LEVEL_LOWEST,j);
		}
		NONE = colors[0];
		GRASS = colors[6];
		SAND = colors[10];
