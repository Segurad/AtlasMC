...
class ConcurrentLinkedListBasicTest {
	...
	private void assertInvariants(ConcurrentLinkedList<String> list) {
		...
		while (it.hasNext()) {
			...
			count++;
			assertTrue(count <= list.size(), "Iterator appears to loop forever.");
		}
	...
	void testAddMultipleElements() {
		...
		assertIterableEquals(List.of("A", "B", "C", "D"), iterate(list));
	...
	void testRepeatedAddFirst() {
		...
		assertEquals("A", list.getHead());
		assertEquals("C", list.getTail());
		assertIterableEquals(List.of("A", "B", "C"), iterate(list));
		assertInvariants(list);
	}
	...
	void testMixedAddAndAddFirst() {
		...
		assertIterableEquals(List.of("0", "A", "B", "C", "D"), iterate(list));
	...
	void testRemoveAllDuplicates() {
		ConcurrentLinkedList<String> list = newList("A", "B", "B", "B", "C");
	...
	void testRemovePreservesOrder() {
		...
		list.remove("B");
		list.remove("D");
		assertIterableEquals(List.of("A", "C", "E"), iterate(list));
		assertEquals("A", list.getHead());
	...
	void testAddAfterClear() {
		...
		assertEquals("D", list.getTail());
		assertIterableEquals(List.of("D"), iterate(list));
		assertInvariants(list);
	}
	...
	void testAddFirstAfterClear() {
		...
		assertIterableEquals(List.of("X"), iterate(list));
		assertInvariants(list);
	}
